...
export default function ScannerScreen() {
  ...
  useFocusEffect(useCallback(() => {
    let active = true;
    loadCredential().then(async (credential) => {
      if (!credential) return active && setPaired(false);
      try { const current = await getPairStatus(credential); if (active) setPaired(current.status === 'paired'); }
  ...
  const sendToPc = async () => {
    ...
    setSending(true);
    try {
      await sendHandoff(result.data);
      setNotice('PCの受信箱へ送信しました。PC側で内容を確認してから開けます。');
    } catch (error) {
      if (error instanceof Error && ['unauthorized', 'not_paired'].includes((error as { code?: string }).code ?? '')) setPaired(false);
      setNotice(error instanceof Error ? error.message : 'PCへ送信できませんでした。');
    ...
    <ScrollView contentInsetAdjustmentBehavior="automatic" contentContainerStyle={styles.content}>
      ...
      {result && (
        <View style={styles.resultCard}>
          ...
          {paired ? <Pressable disabled={sending} style={styles.fullPrimaryAction} onPress={sendToPc}><Text style={styles.primaryButtonText}>{sending ? 'PCへ送信中…' : 'PCへ送る'}</Text></Pressable> : <Pressable style={styles.pairAction} onPress={() => router.push('/pair')}><Text style={styles.secondaryActionText}>PCと連携して送る</Text></Pressable>}
      ...
      <Pressable style={styles.pairLink} onPress={() => router.push('/pair')}><Text style={styles.pairLinkText}>{paired ? 'PC連携を確認' : 'PCと連携'}</Text></Pressable>
      <Text selectable style={styles.privacy}>連携後も、PCではリンクを自動で開かず、内容を確認してから操作します。</Text>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  fullPrimaryAction: { minHeight: 48, alignItems: 'center', justifyContent: 'center', borderRadius: 12, borderCurve: 'continuous', backgroundColor: '#1463F3' },
  pairAction: { minHeight: 48, alignItems: 'center', justifyContent: 'center', borderRadius: 12, borderCurve: 'continuous', borderWidth: 1, borderColor: '#1463F3', backgroundColor: '#FFFFFF' },
  ...
  pairLink: { minHeight: 36, alignItems: 'center', justifyContent: 'center' },
  pairLinkText: { color: '#125EE9', fontSize: 14, fontWeight: '700' },
